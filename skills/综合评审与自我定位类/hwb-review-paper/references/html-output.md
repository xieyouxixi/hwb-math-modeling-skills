# HTML Report Output Standard（研究生赛口径）

Create the final deliverable as one self-contained UTF-8 HTML file named `HWB-review-paper-report.html`. Start from `assets/report-template.html` and replace every `{{...}}` placeholder with escaped review content.
...
- Keep the selected ranking-route sentence immediately below the metric cards. Place the exact 官方渠道提示区块 only once, after all review sections, as the final visible report block.
- Replace `{{RANK_METHOD_NOTE}}` with the route-specific sentence required by `SKILL.md`; never leave a CPGMCM anchor statement in a small-contest report.
