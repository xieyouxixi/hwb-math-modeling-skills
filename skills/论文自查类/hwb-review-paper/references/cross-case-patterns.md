...
The Skill knowledge base was distilled from the scoring rules, scoring points, review summaries and complete review workflows of 12 Huawei Cup (CPGMCM) problems from 2020-2025. Bundled calibration files may expose only a task-relevant subset. Treat patterns supported by at least two compatible cases as provisional transferable rules, and treat CPGMCM-derived three-dimensional competition calibration (problem difficulty, participating unit, Huawei problem) as inapplicable to other contests.
...
5. Declare every borrowed pattern and its confidence. The 12-problem CPGMCM knowledge base supports broader provisional rules, not universal contest laws or non-CPGMCM ranking distributions.
